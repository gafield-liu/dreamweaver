...
import { VideoWithMuteButton } from './hero-video-with-mute';
...
export function Hero({
  ...
  return (
    <section
      id={section.id}
      className={cn(
        `relative pt-24 pb-8 md:pt-36 md:pb-8`,
        section.className,
        className
      )}
    >
            ...
            <div className="border-x">
              ...
              {section.video?.src ? (
                (() => {
                  ...
                  const videoWithGrid = video as { grid?: string; sources?: [string, string, string, string] };
                  const is2x2 = videoWithGrid.grid === '2x2';
                  if (is2x2) {
                    return (
                      <div className="border-border/25 relative z-2 grid grid-cols-2 gap-1 border bg-muted/30 p-1">
                        {([0, 1, 2, 3] as const).map((i) => {
                          const videoSrc = videoWithGrid.sources?.[i] ?? video.src;
                          return (
                            <VideoWithMuteButton
                              key={i}
                              src={videoSrc}
                              poster={video.poster || section.image?.src || section.image_invert?.src}
                              autoPlay={video.autoplay !== false}
                              loop={video.loop !== false}
                              muted={video.muted !== false}
                              playsInline={video.playsInline !== false}
                              ariaLabel={i === 0 ? (video.alt || section.image?.alt || '') : undefined}
                  ...
                  return (
                    <VideoWithMuteButton
                      src={video.src}
                      poster={video.poster || section.image?.src || section.image_invert?.src}
                      width={video.width || section.image?.width || 1200}
                      height={video.height || section.image?.height || 630}
                      autoPlay={video.autoplay !== false}
                      loop={video.loop !== false}
                      muted={video.muted !== false}
                      playsInline={video.playsInline !== false}
                      ariaLabel={video.alt || section.image?.alt || ''}
