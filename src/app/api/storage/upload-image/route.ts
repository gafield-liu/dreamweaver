...
export async function POST(req: Request) {
  try {
    ...
    // R2: without Domain, the URL is the S3 API endpoint and cannot be opened in browser
    const hasUnviewableR2Url = uploadResults.some(
      (r) => r.url && r.url.includes('r2.cloudflarestorage.com')
    );
    if (hasUnviewableR2Url) {
      return respErr(
        'R2 Domain is not set. Please fill in Domain with your R2 public URL (e.g. https://pub-xxxx.r2.dev) in Admin → Settings → Storage.'
      );
    }
    ...
    const message =
      e instanceof Error && e.message === 'No storage provider configured'
        ? 'No storage provider configured. Please configure Cloudflare R2 or AWS S3 in Admin → Settings → Storage.'
        : 'upload image failed';
