...
export default async function AiTasksPage({
  ...
  searchParams: Promise<{ page?: number; pageSize?: number; type?: string; scene?: string }>;
}) {
  ...
  const { page: pageNum, pageSize, type, scene } = await searchParams;
  ...
  const tabs: Tab[] = [
    {
      ...
      name: 'all',
      url: '/admin/ai-tasks',
      is_active: !type && !scene,
    },
    {
      title: t('list.tabs.story'),
      name: 'story',
      url: '/admin/ai-tasks?scene=create-book',
      is_active: scene === 'create-book',
    },
    {
    ...
    },
    {
      ...
      is_active: type === 'text' && scene !== 'create-book',
    },
  ];
