...
export function getMetadata(
  ...
  return async function generateMetadata({
    ...
    // alternates.languages for hreflang (same page in other locales)
    const path = !options.canonicalUrl ? '/' : options.canonicalUrl.startsWith('/') ? options.canonicalUrl : `/${options.canonicalUrl}`;
    const languages: Record<string, string> = {};
    if (locales?.length) {
      for (const loc of locales) {
        const base = `${envConfigs.app_url}${loc === defaultLocale ? '' : `/${loc}`}`;
        const pathForLocale = path === '/' && loc !== defaultLocale ? '' : path;
        const full = pathForLocale ? `${base}${pathForLocale}` : base;
        languages[loc] = full.endsWith('/') && loc !== defaultLocale ? full.slice(0, -1) : full;
      }
    }
