...
export function MusicGenerator({ className, srOnlyTitle }: SongGeneratorProps) {
  ...
  // TTS / Voice clone state
  const [activeMainTab, setActiveMainTab] = useState<'music' | 'tts'>('music');
  ...
  const handleSpeakerAudioChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (!file.type.startsWith('audio/')) {
      toast.error(t('generator.tts.speaker_audio_invalid'));
      return;
    }
    if (file.size > 15 * 1024 * 1024) {
      toast.error(t('generator.tts.speaker_audio_too_large'));
      return;
    }
    setSpeakerAudioFile(file);
    ...
    try {
      ...
      const resp = await fetch('/api/storage/upload-audio', {
        method: 'POST',
        body: formData,
      });
      if (!resp.ok) throw new Error('Upload failed');
      const data = await resp.json();
      if (data.code !== 0 || !data.data?.urls?.length) throw new Error(data.message || 'Upload failed');
      setSpeakerAudioUrl(data.data.urls[0]);
      toast.success(t('generator.tts.speaker_upload_success'));
    } catch (err: any) {
      toast.error(t('generator.tts.speaker_upload_failed'));
    } finally {
  ...
  const handleGenerateTts = async () => {
    ...
    const usePresetVoice = ttsVoice === 'Rachel';
    if (!usePresetVoice && !speakerAudioUrl) {
      toast.error(t('generator.tts.upload_speaker_first'));
      return;
    }
    if (!ttsText.trim()) {
      toast.error(t('generator.tts.enter_text'));
      return;
    }
    ...
    try {
      const resp = await fetch('/api/ai/tts', {
        ...
        body: JSON.stringify({
          text: ttsText.trim(),
          ...(usePresetVoice ? { voice: 'Rachel' } : { speakerAudioUrl }),
        }),
      });
      const data = await resp.json();
      if (resp.ok && data.code === 0 && data.data?.url) {
        setGeneratedTtsUrl(data.data.url);
        toast.success(t('generator.tts.generate_success'));
      } else {
        throw new Error(data.message || 'TTS not available');
      }
    } catch (err: any) {
      ...
      if (
        ...
        toast.info(t('generator.tts.coming_soon'));
      } else {
        toast.error(t('generator.tts.generate_failed'));
      }
    } finally {
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                <Tabs value={activeMainTab} onValueChange={(v) => setActiveMainTab(v as 'music' | 'tts')}>
                  <TabsList className="bg-primary/10 grid w-full grid-cols-2">
                    <TabsTrigger value="music">
                      <Music className="mr-2 h-4 w-4" />
                      {t('generator.tabs.music')}
                    </TabsTrigger>
                    <TabsTrigger value="tts">
                      <Mic className="mr-2 h-4 w-4" />
                      {t('generator.tabs.tts')}
                    </TabsTrigger>
                  </TabsList>
                ...
                {isGenerating && (
                  ...
                  {activeMainTab === 'tts' && (
                    <div className="mt-6 space-y-6">
                      <div className="space-y-2">
                        <Label>{t('generator.tts.voice')}</Label>
                        <p className="text-muted-foreground text-sm">
                          {t('generator.tts.voice_rachel_hint')}
                        </p>
                        <select
                      ...
                      <div className="space-y-2">
                        <Label>{t('generator.tts.speaker_audio')} ({t('generator.tts.optional')})</Label>
                        <p className="text-muted-foreground text-sm">
                          {t('generator.tts.speaker_audio_hint')}
                        </p>
                        <div className="flex items-center gap-3">
                          <Input
                            ...
                            onChange={handleSpeakerAudioChange}
                        ...
                        {(speakerAudioUrl || speakerAudioFile) && (
                          <p className="text-muted-foreground text-xs">
                            {speakerAudioFile?.name ?? t('generator.tts.speaker_uploaded')}
                          </p>
                        )}
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="tts-text">{t('generator.tts.text_to_speak')}</Label>
                        <Textarea
                          id="tts-text"
                          placeholder={t('generator.tts.text_placeholder')}
                          value={ttsText}
                      ...
                      {!user ? (
                        ...
                        <Button
                          ...
                          disabled={isGeneratingTts || (ttsVoice !== 'Rachel' && !speakerAudioUrl) || !ttsText.trim()}
                        >
                          {isGeneratingTts ? (
                            <>
                              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                              {t('generator.tts.generating')}
                            </>
                          ) : (
                            <>
                              <Mic className="mr-2 h-4 w-4" />
                              {t('generator.tts.generate')}
                            </>
                          )}
                        </Button>
                      )}
                ...
                <CardTitle className="flex items-center gap-2">
                  {activeMainTab === 'tts' ? (
                    <>
                      ...
                      {t('generator.tts.generated_speech')}
                    </>
                  ) : (
                    <>
                    ...
                    <div className="space-y-4">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          a.href = generatedTtsUrl;
                          a.download = 'tts-speech.mp3';
                          a.click();
                        }}
                      >
                        <Download className="mr-2 h-4 w-4" />
                        {t('generator.downloading')}
                      </Button>
                    </div>
                  ) : (
