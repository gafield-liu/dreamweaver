...
export class KieProvider implements AIProvider {
  ...
  async generateVideo({
    ...
    // build request params for Seedance-style API: input_urls, aspect_ratio 16:9, resolution 480p, duration 4
    let payload: any = {
      model: params.model,
      callBackUrl: params.callbackUrl,
      input: {
    ...
    if (params.options) {
      const options = params.options;
      // image-to-video: use input_urls (Seedance / bytedance/seedance-1.5-pro)
      if (options.image_input && Array.isArray(options.image_input)) {
  ...
  /** TTS (text-to-speech) via Kie ElevenLabs model. No voice clone; uses preset voice e.g. Rachel. */
  async generateTts({
    params,
  }: {
    params: AIGenerateParams;
    ...
    const model = params.model || 'elevenlabs/text-to-speech-turbo-2-5';
    const text = params.prompt || (params.options?.text as string) || '';
    if (!text.trim()) {
      throw new Error('text is required for TTS');
    }

    const voice = (params.options?.voice as string) || 'Rachel';
    const payload = {
      model,
      callBackUrl: params.callbackUrl,
      input: {
        ...
        timestamps: params.options?.timestamps ?? false,
      },
    };
    ...
    const { code, msg, data } = await resp.json();
    if (code !== 200) {
      throw new Error(`generate TTS failed: ${msg}`);
    }
    if (!data?.taskId) {
      throw new Error('generate TTS failed: no taskId');
    }
  ...
  async queryTts({ taskId }: { taskId: string }): Promise<AITaskResult> {
    ...
    const { code, msg, data } = await resp.json();
    if (code !== 200) {
      throw new Error(msg || 'query TTS failed');
    }
    if (!data?.state) {
      throw new Error('query TTS failed: no state');
    }
    ...
    if (data.resultJson) {
      try {
        ...
        audioUrl =
          ...
          (Array.isArray(resultJson.resultUrls) ? resultJson.resultUrls[0] : undefined);
      } catch {
        // ignore parse error
      }
    }
