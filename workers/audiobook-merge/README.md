...
```bash
cd workers/audiobook-merge
docker build -t audiobook-merge .
...
1. **在别处部署本目录的 Docker**（任选其一即可）  
   - [Railway](https://railway.app)：新建 Project → Deploy from Dockerfile，根目录选仓库里的 `workers/audiobook-merge`，填好下方 R2 / `WORKER_SECRET` 等环境变量，部署后会得到公网域名。  
